...
impl Camera {
    pub fn new(options: CameraOptions) -> Self {
        let aspect_ration: f32 = match options.aspect_ratio {
            AspectRatios::Widescreen => 16.0 / 9.0,
            AspectRatios::Square => 1.0,
            AspectRatios::Smartphone => 9.0 / 16.0,
            AspectRatios::Standard => 4.0 / 3.0,
            AspectRatios::Cinema => 1.85,
        };

        let image_width = options.image_width;
        let image_height: u32 = std::cmp::max(1, (image_width as f32 / aspect_ration) as u32);
